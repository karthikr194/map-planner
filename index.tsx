...
*/
// Fix: Add reference to google.maps types to resolve 'google' not found errors.
/// <reference types="@types/google.maps" />
import {FunctionDeclaration, GoogleGenAI, Type} from '@google/genai';

// Fix: Add a global declaration for window.Popup to address type errors.
declare global {
  interface Window {
    Popup: any;
  }
...
let activeLinePopup = null; // Holds the popup for a clicked route
...
async function initMap() {
  ...
  map.addListener('click', closeActiveLinePopup);
  ...
  window.Popup = class Popup extends google.maps.OverlayView {
    ...
    constructor(position, content) {
      ...
      // Fix: Use google.maps.OverlayView directly for the static method call.
...
// Closes the currently active popup for a route line.
function closeActiveLinePopup() {
  if (activeLinePopup) {
    activeLinePopup.setMap(null);
    activeLinePopup = null;
  }
}
...
async function setLeg(args) {
  ...
  geodesicPoly.addListener('click', (event: google.maps.PolyMouseEvent) => {
    closeActiveLinePopup(); // Close existing popup

    const content = document.createElement('div');
    let legDetails = `<b>${args.name}</b>`;
    if (args.transport || args.travelTime) legDetails += '<br/>';
    if (args.transport) {
      legDetails += `<i class="fas fa-${getTransportIcon(args.transport)}"></i> ${
        args.transport
      }`;
    }
    if (args.travelTime) {
      legDetails += `${args.transport ? ' &bull; ' : ''}${args.travelTime}`;
    }
    content.innerHTML = legDetails;

    const popup = new window.Popup(event.latLng, content);
    popup.setMap(map);
...
function highlightCard(index: number) {
  ...
  // Fix: Specify HTMLElement for querySelectorAll to access properties like offsetWidth and offsetLeft.
